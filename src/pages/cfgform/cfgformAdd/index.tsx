    ...
    },
    {
      ...
      render: (value: any, row: any, index: any) => (
        <Space size="middle">
          ...
          >
            {/* <Button type="default" danger onClick={() => setVisible(true)}>
              删除
            </Button> */}
  ...
  const submitSelect = (value: any) => {
    ...
    for (let i = 0; i < state.assembly_main.length; i++) {
      if (state.assembly_main[i].id == value.selectMain) {
        ...
        // console.log('大类:' + JSON.stringify(state.assembly_main[i]));
        break;
      }
    }
    for (let i = 0; i < state.assembly_sub.length; i++) {
      if (state.assembly_sub[i].id == value.selectSub) {
        ...
        // console.log('部件:' + JSON.stringify(state.assembly_sub[i]));
        break;
      }
    }
    for (let i = 0; i < state.assembly_model.length; i++) {
      if (state.assembly_model[i].id == value.selectType) {
        ...
        // console.log('型号:' + JSON.stringify(state.assembly_model[i]));
        break;
      }
    }
        ...
        Request(
          ...
          (data: any) => {
            ...
            if (data.code === '00') {
              ...
              handleSaveClose();
            } else if (data.code === '99') message.error('此编号id已存在');
  ...
  const switchChange = (checked: any, type: any, index: any) => {
    ...
    switch (type) {
      ...
      case 'light':
        ...
        if (!checked) {
          // for(let i=0;i<selectedItems.length;i++){
          //   if(selectedItems[i]===selectIndex[index].Light)
          //   selectedItems.splice(i,1)
          // }
          // if(selectIndex[index]) selectIndex[index].Light = '';
          // console.log(selectIndex)
          // let temp = state.selectLight
